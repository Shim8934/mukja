...
<section class="hero-wrap hero-wrap-2 align-items-center"
	...
	<div class="container">
		<div
			...
			<div class="col-md-9 ftco-animate text-center">
				...
				<p class="breadcrumbs">
					<span class="mr-2"><a href="index.html">My Page <i
			...
			<div class="text-center">
				...
				<div class="col-md-5 mt text-center heading-section ftco-animate"
					...
					<div class="border3bc">
						<div class="sub-title">
							...
							<input type="hidden" name="u_nick" id="u_nick"
								value="${myInfo.u_nick}">
								...
								<c:if test="${empty myInfo.u_img}">
									<div style="margin-top: 1rem;">
										<div class="col-md-10 col-offset-1"
											style="background-position: center; background-image: url(<c:url value=""/>);"></div>
									</div>
								</c:if>
											...
											<c:if test="${myJjim.store_name == storetxt.username}">
												<div class="mpjjim item-align-center bd2bc"
													...
													<div style="background-color: white; border-radius: 50%">
														<input type="hidden" id="ms_no" name="ms_no"
															value="${myJjim.ms_no}"> <a
															href='<c:url value="/deleteMyJjim.bbs?ms_no=${myJjim.ms_no}"/>'>
					...
					<!-- 리뷰 시작 -->
					<div id="RVList" class="row reviews border3bc mplist" style="margin-right: 0px; margin-left: 0px;">
						<div class="sub-title">
												...
												<c:if test="${rvcnt.store_name == storetxt.username}">
													<tr>
														<td><a
															href='<c:url value="/Store/DetailView.do?username=${storetxt.username}"/>'
															style="text-decoration: none;">
																${storetxt.store_name} </a> <br /> <c:forEach
																items="${menus}" var="menus" varStatus="loop">
														...
														<td><c:if test="${empty rvimgs}">
																<img class="rv_pics" />
															</c:if> <c:if test="${not empty rvimgs}">
																<%
																	int count = 0;
																%>
														...
														<td>
															<input type="hidden" class="reviews" id="rv_no" value="${rvcnt.rv_no}">	
															<a href="<c:url value='/updateMyReview.bbs?rv_no=${rvcnt.rv_no}'/>" class="btn btn-warning" style="font-size: 12px; padding: 3px 6px;">수정</a> 
															<input type="button" class="reviewdelete btn btn-primary" style="font-size: 12px; padding: 2px 4px;" value="삭제">																						
														</td>
													</tr>
												</c:if>
											</c:forEach>
				...
				<div class="col-md-5 et text-center heading-section ftco-animate"
					...
					<div class="row groups border3bc mplist" style="margin-right: 0px; margin-left: 0px; ">
						...
						<div class="container">
							<div class="row" style="margin-right: 0px; margin-left: 0px; overflow:auto;">
								<table
									class="mplist table table-bordered table-hover text-center item-a">
									...
									<c:if test="${not empty myET0}">
										<c:forEach items="${myET0}" var="ET0" varStatus="loopout">
											<c:forEach items="${storetxt}" var="storetxt"
												varStatus="loopin">
												<c:if test="${ET0.store_id == storetxt.username}">
													<tr>
														<td>
														<input type="hidden" name="er_no" class="er_no"value="${ET0.er_no}"> 
														<input type="hidden" name="user_id" id="user_id" value="${ET0.user_id}">
															<input type="button" class="allok btn btn-primary" style="font-size: 12px; padding: 2px 4px;" value="수락">
															<input type="button" class="allno btn btn-warning" style="font-size: 12px; padding: 2px 4px;" value="거절">
														<td style="font-weight: bold;">
															<c:forEach items="${Nicks0}" var="Nick0" varStatus="loop">
																	<c:if test="${ET0.applyer == Nick0.username}">
																		<input type="hidden" name="applyer" class="applyer" value="${Nick0.username}"> ${Nick0.u_nick}
																	</c:if>
															</c:forEach>
														</td>
														...
														<td>${ET0.er_time}</td>
														<td style="font-size: 18px;"><c:forEach
																items="${Nicks}" var="Nick" varStatus="loop">
																<c:if test="${ET0.er_no == Nick.er_no}">
																	<span class="overflow"> <a
																		href='<c:url value="/MyPage.bbs?user_id=${Nick.username}"/>'>
																			${Nick.u_nick} </a>
																	</span>
																</c:if>
															</c:forEach></td>
														<c:forEach items="${etInCount}" var="etcnt"
															varStatus="loop">
					...
					<div class="groups border3bc col-md-12">
						...
						<div class="owl-carousel col-md-12 pt-3"
							style="padding: 0px; margin: 0px;">
							<c:if test="${empty myET1}">
								<div class="mpjjim bd2bc col-md-12"
									style="float: none; background: lightblue;">
								...
								<c:forEach items="${myET1}" var="myET1" varStatus="loop">
									<c:forEach items="${storetxt}" var="storetxt" varStatus="loop">
										<c:if test="${myET1.username == storetxt.username}">
											...
											<div class="mpjjim bd2bc pb-3 pt-3 col-md-11"
												style="float: none; background: lightblue; display: inline-block;">
												<div class="img col-md-6"
													style="display: inline-block; padding: 5px; margin: 0px;">
													<c:forEach items="${storeimgs}" var="storeimgs"
														varStatus="loop">
														<c:if test="${storeimgs.username == myET1}">
															<c:if test="${empty storeimgs.sf_path}">
																<div class="img rv_list_img col-md-12"
															...
															<c:if test="${not empty storeimgs.sf_path}">
																<div class="img rv_list_img col-md-12"
																	style="background-image: url(<c:url value="${storeimgs.sf_path}"/>);">
																</div>
															</c:if>
														</c:if>
													</c:forEach>
												...
												<div class="text col-md-6"
													style="display: inline-block; padding: 5px; margin: 0px; vertical-align: top;">
													<div>
													...
													<c:forEach items="${etInCount}" var="etcnt"
														...
														<c:if test="${myET1.er_no == etcnt.er_no}">
															<div class="overflow">${etcnt.count} /
																${myET1.er_max} 명</div>
														</c:if>
													</c:forEach>
													<div class="">
														<c:forEach items="${Nicks}" var="Nick" varStatus="loop">
															<c:if test="${myET1.er_no == Nick.er_no}">
																<span class="overflow"> <c:if
																		test="${myET1.user_id != Nick.username}">
																		<a onclick="addr(this)"
												...
												<c:if test="${myET1.user_id == myET1.er_master}">
													<div class="col-md-12 pt-3">
														<input type="hidden" class="ermDel" value="${myET1.er_no}">
														<input type="button" class="eroomDel btn btn-danger" value="삭제">
														<a href="<c:url value='/userReport.bbs?er_no=${myET1.er_no}'/>" class="btn btn-black"> 회원신고 </a>
													</div>
												</c:if>
												<c:if test="${myET1.user_id != myET1.er_master}">
													<div class="col-md-12 pt-3">
														<a href="<c:url value="/erDelete.bbs?er_no=${myET1.er_no}"/>" class="btn btn-warning" id="erDelete" style="cursor: pointer; color: white;">나가기</a>
														<a href="<c:url value='/userReport.bbs?er_no=${myET1.er_no}'/>" class="btn btn-black"> 회원신고 </a>
...
//리뷰 삭제 
$(function(){			       
	var rvs = document.querySelectorAll(".reviewdelete");
	rvs.forEach(function(item, index){
		item.addEventListener('click', function(event){
			 var rv_no = $(".reviews").eq(index+1).val();
			 console.log(rv_no);
	 	        $.ajax({
				url : "<c:url value='/deleteMyReview.bbs?rv_no="+rv_no+"'/>",
				data : {
					"rv_no" : rv_no,
				},
				dataType : 'json',
...
//채팅방 삭제 버튼
$(function(){
	var eroomDel = document.querySelectorAll(".eroomDel");		
	eroomDel.forEach(function(item, index){ 		
		item.addEventListener('click', function(event){		       
	        var er_no = $(".ermDel").eq(index).val();
	        $.ajax({
					url : "<c:url value='/deleteMyETHist.bbs?er_no="+er_no+"'/>",
					data : {"er_no" : er_no},
					dataType : 'json',
					success : function(data) {
						console.log('성공..?:', data);
...
//채팅방 참가 수락버튼			
	$(function(){
		var allOk = document.querySelectorAll(".allok");
		allOk.forEach(function(item, index){ 
			item.addEventListener('click', function(event){		       
		        var er_no = $(".er_no").eq(index).val();
				var applyer = $(".applyer").eq(index).val();
		        $.ajax({
...
//채팅방 신청 거절 버튼
	$(function(){
		var allNo = document.querySelectorAll(".allno");		
		allNo.forEach(function(item, index){ 		
			item.addEventListener('click', function(event){		       
		        var er_no = $(".er_no").eq(index).val();
				var applyer = $(".applyer").eq(index).val();
		        $.ajax({
						url : "<c:url value='/er_Reject.bbs?er_no="+ er_no+ "&applyer="+ applyer + "'/>",
						data : {
							"er_no" : er_no,
							"applyer" : applyer
						},
		...
		$.ajax({
					...
					success : function(data) {
						...
						if (data != null) {
							...
							var tend_img = '';
							console.log('4');
							var prof = '<div id="showuser"class="myborder infoWindow" style=" display:none;">'
									+ ' <img style="margin-bottom:20px; display:inline;" src="/mukja'+data.u_img+'" alt="H" class="img_user">'
									+ ' <div class="infoText">닉네임 : '
									+ data.u_nick
									+ '</div>'
									+ ' <div class="infoText">나이:'
									+ data.u_age
									+ '대</div>'
							...
							for (var ti = 0; ti < tend_codes.length; ti++) {
								for (var ut = 0; ut < user_tend.length; ut++) {
									if (tend_codes[ti] == user_tend[ut]) {
										console.log(data.er_title + '의 성향이미지:'
												+ user_tend[ut]);
